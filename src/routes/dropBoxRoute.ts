...
router.get("/save", (request: Request, response: Response) => {
  response.status(200).json({
    message:
      "Please send a post request with your file for temporary storage. Don't forget to add your api-key as an [x-api-key] header. Dont have one? Make a GET request to /api/auth for more information.",
    note: "The file will be deleted after the time duration has been exceeded.",
    params: {
      duration:
        "Amount of days to keep the image before discarding [optional, default = 1]",
    },
  });
});
