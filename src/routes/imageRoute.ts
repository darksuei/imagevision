...
router.get("/image-classification", (req: Request, res: Response) => {
  const response = {
    status: "Success",
    message:
      "Have an API KEY? Send a POST request to this endpoint to classify an image. Don't have one yet? Make a GET request to /api/auth for more information.",
...
router.post(
  ...
  async (req: any, res: Response) => {
    ...
    if (req.body.confidenceThreshold) {
      if (
        req.body.confidenceThreshold < 0 ||
        req.body.confidenceThreshold > 1
      ) {
        return res
          .status(400)
